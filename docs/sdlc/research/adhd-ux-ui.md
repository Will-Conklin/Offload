...
- **Capture flows (Features/Captures, App/MainTabView):** Keep the capture entry point always visible via a primary tab or floating button. Default to immediate save with optional “Organize now” secondary action; auto-focus the text area and show voice input as an equal first-class option.
- **Captures list (Features/Captures):** Present items in a calm list with light dividers and concise metadata (timestamp, type). Include batch-select + “Move to…” with undo rather than confirmations. Surface a “Ready to organize” chip when there are unfiled items.
