...
## Context
Recent research synthesized ADHD-friendly UX and UI principles for Offload. To make this guidance actionable and traceable, we need explicit decisions that constrain design and implementation across capture, organize flows, and the design system.
...
4. **Predictable navigation:** Core areas (Captures, Plans, Lists, Settings) remain one tap away via the main tab shell. Capture uses sheets; editing uses full screens. Swipe actions are mirrored with visible buttons.
...
- Undo infrastructure is required across Capture and Organize flows, influencing data and UI layers.
